...
export const BluetoothDeviceItem = GObject.registerClass({
    ...
    constructor(toggle, device, batteryInfoReported, batteryEnabled, deviceIcon) {
        super({
        ...
        this.connectObject('destroy', () => {
            ...
            if (this._stateSignalId)
                this.disconnect(this._stateSignalId);
    ...
    updateProps(batteryEnabled, deviceIcon) {
        this._batteryEnabled = batteryEnabled;
        ...
        if (this._iconType !== deviceIcon)
            this._iconType = deviceIcon;
        this._icon.icon_name = `${this._iconType}-symbolic`;
    }

    _addBatterySupportedDevice() {
        this._toggle._addNewDeviceToList(this._device);
