...
var BluetoothBatteryMeter = GObject.registerClass({
    ...
    _startToggle() {
        ...
        this._deviceTimeMap = new Map();
        this._pullDeviceTimeFromGsetting();
    ...
    *getDevices() {
    ...
    _reorderDeviceItems() {
        const devices = this._getRecencySortedDevices();
    ...
    _getRecencySortedDevices() {
        ...
        for (const dev of devices) {
            ...
            if (this._deviceTimeMap.has(path)) {
                const props = this._deviceTimeMap.get(path);
                if (dev.connected !== props.connected) {
                    props.connected = dev.connected;
                    props[dev.connected ? 'timeConnected' : 'timePaired'] = currentTime;
                    updateGsetting = true;
                }
            } else {
                const props = {
                    'connected': true,
                    'timeConnected': currentTime,
                    'timePaired': currentTime,
                };
                this._deviceTimeMap.set(path, props);
                updateGsetting = true;
            }
        }

        const devicePaths = devices.map(dev => dev.get_object_path());
        for (const [path] of this._deviceTimeMap) {
            if (!devicePaths.includes(path)) {
                this._deviceTimeMap.delete(path);
                updateGsetting = true;
            }
        }

        if (updateGsetting)
            this._pushDevicesTimeToGsetting();

        return !this._sortDevicesByHistory ? devices : devices.sort((dev1, dev2) => {
            const path1 = dev1.get_object_path();
            const path2 = dev2.get_object_path();

            const props1 = this._deviceTimeMap.get(path1);
            const props2 = this._deviceTimeMap.get(path2);

            if (props1.connected !== props2.connected)
                return props2.connected - props1.connected;


            if (props1.connected)
                return props2.timeConnected - props1.timeConnected;

            return props2.timePaired - props1.timePaired;
        });
    }

    _pullDeviceTimeFromGsetting() {
        this._deviceTimeMap.clear();
        ...
        if (deviceList.length !== 0) {
            for (const jsonString of deviceList) {
                ...
                const props = {
                    ...
                    'timeConnected': item['time-connected'],
                    'timePaired': item['time-paired'],
                };
                this._deviceTimeMap.set(path, props);
            }
        }
    ...
    _pushDevicesTimeToGsetting() {
        const deviceList = [];
        for (const [path, props] of this._deviceTimeMap) {
            const item = {
                path,
                'connected': props.connected,
                'time-connected': props.timeConnected,
                'time-paired': props.timePaired,
            };
