...
export const BluetoothBatteryMeter = GObject.registerClass({
        ...
        if (deviceList.length !== 0) {
            for (const jsonString of deviceList) {
                ...
                const props = {
                    ...
                    'alias': item['alias'],
                    'paired': item['paired'],
                    'batterySupported': item['battery-supported'],
    ...
    _pushDevicesToGsetting() {
        ...
        for (const [path, props] of this._deviceList) {
            const item = {
                ...
                'alias': props.alias,
                'paired': props.paired,
                'battery-supported': props.batterySupported,
    ...
    _addNewDeviceToList(device, batteryReported) {
        ...
        const props = {
            ...
            alias: device.alias,
            paired: true,
            batterySupported: batteryReported,
            batteryEnabled: true,
        ...
        for (const dev of devices) {
            ...
            if (this._deviceItems.has(path)) {
                if (this._deviceList.has(path)) {
                    ...
                    item.updateProps(this._deviceList.get(path).batteryEnabled, this._deviceList.get(path).icon);
                }
                continue;
            }
            if (this._removedDeviceList.length > 0) {
            ...
            if (this._deviceList.has(path)) {
                ...
                batteryInfoReported = props.batterySupported;
            } else {
                deviceIcon = dev.icon;
                batteryInfoReported = dev.battery_percentage > 0;
                this._addNewDeviceToList(dev, batteryInfoReported);
            }

            const batteryEnabled = batteryInfoReported ? props.batteryEnabled : false;

            const item = new BluetoothDeviceItem(this, dev, batteryInfoReported, batteryEnabled, deviceIcon);
        ...
        if (this._settings.get_boolean('enable-battery-indicator')) {
            for (const dev of devices) {
                ...
                if (this._deviceIndicators.has(path)) {
                    if (!dev.connected || !this._deviceList.get(path).indicatorEnabled) {
                        this._deviceIndicators.get(path)?.destroy();
                        this._deviceIndicators.delete(path);
                    }
                    continue;
                ...
                if (!dev.connected)
                    continue;
                if (this._deviceList.has(path) && this._deviceList.get(path).indicatorEnabled) {
                    const indicator = new BluetoothIndicator(this._settings, dev, this._deviceList.get(path).icon, this._widgetInfo);
