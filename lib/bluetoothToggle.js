...
export const BluetoothBatteryMeter = GObject.registerClass({
    ...
    _checkToggle() {
        // if bluetooth is turned off, wait until it is turned on to get accent color (checked) from Bluettooth toggle button
        if (this._bluetoothToggle.checked) {
            this._getColor();
        } else {
            this._toggleConnectid = this._bluetoothToggle.connect('notify::checked', () => {
                if (this._bluetoothToggle.checked) {
                    this._getColor();
                    this._bluetoothToggle.disconnect(this._toggleConnectid);
                    this._toggleConnectid = null;
                }
            });
        }
    }
    ...
    _getColor() {
        // Usage of 'shell-link' styleclass on paired Icon (which I like and prefer), doesnot work with custom themes as they do not convert all styleclass.
        // This applies accent color with lighten 20% (if theme is dark) or darken 10% (if theme is light) to the paired Icon in quick setting,
        // Similar to how shell-link styleclass behaved.
